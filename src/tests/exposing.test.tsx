...
describe("validate", () => {
    test("should expose info about every validated property", () => {
        ...
        expect(checker.props()["testProp3.withDot"].value).toBe(false);
        expect(typeof checker.props()["testProp3.withDot"].change).toBe("function");
        expect(checker.props()["testProp3.withDot"].errors).toEqual([]);
        expect(typeof checker.props()["testProp3.withDot"].validate).toBe("function");
        expect(typeof checker.props()["testProp3.withDot"].cleanErrors).toBe("function");
    ...
    test("should expose property value for value initialized from external prop using that props", () => {
        const propsProvider = mountTestComponent(
            [
                {
                    initialValueFromProps: (props: any) => props.testProp1 + " from function",
                    name: "testProp1",
                    validators: [
                        {
                            fn: (value: any) => value === "some value",
        ...
        expect(checker.props().testProp1.value).toBe("some value from function");
        expect(typeof checker.props().testProp1.change).toBe("function");
        expect(checker.props().testProp1.errors).toEqual([]);
        expect(typeof checker.props().testProp1.validate).toBe("function");
        expect(typeof checker.props().testProp1.cleanErrors).toBe("function");
